...
class BpjsFragmentNew : Fragment(), CekRMView
    ,ListPoliView, JadwalView, JumlahView ,TipeJKNView,CekKepesertaanView, KartuView {

    companion object {
        fun newInstance() = BpjsFragmentNew()
    }
        ...
        rv_card_pasien.adapter= EkartuPasienAdapter(data as List<EkartuResultItem>,
            object : EkartuPasienAdapter.onClickItem{
                override fun clicked(item: EkartuResultItem?) {
                    ...
                    noktp.setText(item?.cust_usr_nik)
                    nobpjs.setText(item?.custUsrNoJaminan)
                }
            })
    }
